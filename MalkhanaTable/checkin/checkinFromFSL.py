...
def checkin_page(prev_checkin_page):
    global fsl_checkin_frame, barcode_entery, entry_order_no, entry_checkin_date, hour_var, minute_var, text_examiner_report, entry_examiner
    ...
    # Labels
    label_barcode_no = ttk.Label(fsl_checkin_frame, text="Barcode No:", background="#B9E6FF", font=("Helvetica", 12))
    label_order_no = ttk.Label(fsl_checkin_frame, text="Order No:",  background="#B9E6FF",font=("Helvetica", 12))
    label_checkin_time = ttk.Label(fsl_checkin_frame, text="Checkin Time:", background="#B9E6FF", font=("Helvetica", 12))
    label_checkin_date = ttk.Label(fsl_checkin_frame, text="Checkin Date:", background="#B9E6FF", font=("Helvetica", 12))
    label_examiner = ttk.Label(fsl_checkin_frame, text="Examiner Name:",  background="#B9E6FF",font=("Helvetica", 12))
    label_examiner_report = ttk.Label(fsl_checkin_frame, text="Examiner Report:",  background="#B9E6FF",font=("Helvetica", 12))
    ...
    # Entry fields
    barcode_entery = ttk.Entry(fsl_checkin_frame, font=("Helvetica", 12))
    ...
    checkin_date_entery = DateEntry(fsl_checkin_frame, width=12, background='darkblue', foreground='white', borderwidth=2)
    checkin_date_entery.grid(row=3, column=1, padx=5, pady=5, sticky=tk.W)  # Use sticky=tk.W for left alignment
    ...
    # Check-in button
    checkin_button = tk.Button(fsl_checkin_frame, text="Checkin", background="#FFFFFF", command=checkin, font=("Helvetica", 12))
    checkin_button.grid(row=6, column=0, columnspan=2, padx=5, pady=10)

    Home = tk.Button(fsl_checkin_frame, text="Homepage",  background="#FFFFFF",command=go_home, font=("Helvetica", 12))
    Home.grid(row=7, column=0, padx=10, pady=10, sticky=tk.E)

    back_button = tk.Button(fsl_checkin_frame, text="Back",  background="#FFFFFF",command=go_back, font=("Helvetica", 12))
...
def barcode_checker(barcode,date,time,order_no,examiner,examiner_report):
    ...
    if not result:
        messagebox.showerror("Barcode Not Found", "Entered Barcode Not Found In Database.")
...
def already_inornot(barcode,date,time,order_no,examiner,examiner_report):
    ...
    if result and result[0] in ("fsl", "FSL"):
        ...
        log.update_logs(barcode, "Checkin From Fsl", date, time)
